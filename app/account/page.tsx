...
export default function AccountPage() {
  ...
  if (loading) return <main className="min-h-screen pt-20 flex items-center justify-center">Loading...</main>;

  if (!user) {
    return (
      <main className="min-h-screen pt-20">
        <Navbar />
        <section className="py-24 px-6"><AuthForm /></section>
        <Footer />
      </main>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="md:col-span-2 bg-white rounded-[32px] border border-bento-line p-12 flex items-center gap-8">
               ...
               <div className="space-y-1">
                  <h2 className="text-3xl font-black uppercase tracking-tight">{user?.user_metadata?.full_name || 'Member'}</h2>
            ...
            {/* Admin Backlink */}
            <a href="/admin" className="bg-black text-white rounded-[32px] p-8 flex flex-col justify-between hover:bg-black/80 transition-colors group cursor-pointer border border-black">
               <Settings size={24} className="text-white/50 group-hover:text-white transition-colors" />
               <div className="space-y-1 mt-6">
                  <h3 className="text-lg font-black uppercase leading-none">Admin Panel</h3>
                  <p className="text-[9px] font-bold uppercase tracking-widest text-white/60">Manage Store</p>
               </div>
            </a>
            ...
            <div onClick={handleLogout} className="bg-black text-white rounded-[40px] p-8 flex flex-col justify-center items-center gap-4 hover:bg-black/90 transition-all cursor-pointer">
               <LogOut size={24} />
