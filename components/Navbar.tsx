        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex flex-none items-center justify-center gap-10 text-[11px] font-bold uppercase tracking-widest text-black/60">
            ...
            >
              ...
              {/* Dropdown Menu */}
              <div 
                className={`absolute top-20 left-1/2 -translate-x-1/2 bg-white border border-black/10 shadow-lg min-w-[240px] rounded-b-[16px] overflow-hidden transition-all duration-300 origin-top flex flex-col ${isShopHovered ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}
              >
                ...
                {displayCategories.map((cat) => (
                  <Link 
                    ...
                    className="px-6 py-3 hover:bg-gray-50 text-black/60 hover:text-black uppercase tracking-widest text-[10px] transition-colors"
                  >
                    {cat.name}
                  </Link>
                ))}
            ...
            <Link href="/about" className={`hover:text-black transition-colors ${pathname === '/about' ? 'text-black' : ''}`}>Manifesto</Link>
            <Link href="/archives" className={`hover:text-black transition-colors ${pathname === '/archives' ? 'text-black' : ''}`}>Archives</Link>
          </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-20 z-40 bg-white flex flex-col p-6 overflow-y-auto w-full md:hidden">
          <div className="flex flex-col space-y-6 text-xl font-black uppercase tracking-widest text-black">
            ...
            <Link href="/about" onClick={() => setIsMobileMenuOpen(false)}>Manifesto</Link>
